...
public class CCJSqlExample {
	...
	public static void main(String[] args) throws JSQLParserException {
		String varname1 = ""
				+ "SELECT hallprefs.studentname as fdsf "
				+ "    , hh.studentid as dfsd "
				+ "    , hh.studentdesc "
				+ "    , hh.hallname,(select te from ta where fsdf = fsdf) "
				+ "FROM students "
				+ "INNER JOIN hallprefs "
				+ "    on ss.studentid = hallprefs.studentid left outer join fdsff on fsdfsfd = fsff "
				+ "INNER JOIN halls hh "
				+ "    on hp.hallid = hh.hallid cross join (select tst from tsdfs) where gh=jgjh and tsfds = sdfd or fsdfsf = fdsff and (fdsfsf = fdsf or fdsfsf = fdfsf) ";
		...
		for (Iterator<Join> joinsIt = ps.getJoins().iterator(); joinsIt.hasNext();) {
			...
			if(join.getRightItem() != null) {
				String aliase =  join.getRightItem().getAlias() != null ? join.getRightItem().getAlias().getName() : join.getRightItem().toString();
	...
	protected static String getEntityAlias(String entityName, PlainSelect query) {
		/*for (Object o : query.getSelectItems()) {
			SelectItem st = (SelectItem) o;
			if (hasEntityAlias(entityName, st)) {
	...
	/*private static boolean hasEntityAlias(String entityName, SelectItem selectitem) {
		return selectitem instanceof net.sf.jsqlparser.schema.Column
